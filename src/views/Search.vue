  ...
  <div class="dgp-search-main">
    <div class="dgp-search-link clearfix">
      <ButtonGroup class="dgp-search-router">
        <Button>
          <router-link to="/search/all">全部</router-link>
        </Button>
        <Button>
          <router-link to="/search/attention">已关注</router-link>
        </Button>
        <Button>
          <router-link to="/search/tree">树形展示</router-link>
        </Button>
      </ButtonGroup>
    </div>
    <div class="dgp-search-group">
      ...
      <div class="dgp-high-wrap">
        <div class="dgp-search-high">
          <!--高级搜索选择项-->
          <v-search-high-row v-for="(value,key,index) in searchHigh" :key="index" :high-class="value" :high-key="key"></v-search-high-row>
        </div>
      </div>
    </div>
...
<script>
  import VSearchHighRow from "../components/VSearchHighRow";
  export default {
    name: 'search',
    data(){
        return{
